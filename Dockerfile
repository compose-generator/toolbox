...
RUN apk update && apk add --no-cache sudo=1.9.5p2-r0 bash=5.1.0-r0 curl=7.77.0-r0 git=2.30.2-r0 npm=14.16.1-r1 yarn=1.22.10-r0 \
    unzip=6.0-r8 python3=3.8.10-r0 py3-pip=20.3.4-r0 rust=1.47.0-r2 ruby=2.7.3-r0 ruby-dev=2.7.3-r0 make=4.3-r0 zlib-dev=1.2.11-r3 && rm -rf /var/cache/apk/*
...
RUN gem install rails:6.1.3.2 && gem list && gem cleanup

# Install golang
RUN apk add --no-cache --virtual .build-deps gcc=10.2.1_pre1-r3 musl-dev=1.2.2-r0 go=1.15.12-r0 openssl=1.1.1k-r0 && \
    curl -sSL -o go.tar.gz https://golang.org/dl/go1.16.4.src.tar.gz && tar -C /usr/local -xzf go.tar.gz
